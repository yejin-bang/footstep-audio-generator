...
class LoRAWrapper:
    ...
    def configure_optimizers(self):
        """
        Configure optimizer and scheduler for LoRA training.
        Uses the same optimizer_configs as the base model training wrapper.
        Falls back to simple Adam if no config provided.
        """
        
        if self.optimizer_configs is None:
            raise ValueError(
                "LoRAWrapper requires 'optimizer_configs' to be provided.\n"
                "Add 'optimizer_configs' to your model_config.json under 'training' section.\n"
                "Example:\n"
                "  'training': {\n"
                "    'optimizer_configs': {\n"
                "      'diffusion': {\n"
                "        'optimizer': {'type': 'AdamW', 'config': {'lr': 5e-5, ...}},\n"
                "        'scheduler': {'type': 'CosineAnnealingWithWarmup', ...}\n"
                "      }\n"
                "    }\n"
                "  }"
            )
        
        # MAIN PATH: Use optimizer_configs from model config
        if 'diffusion' not in self.optimizer_configs:
            raise KeyError(
                "Expected 'diffusion' key in optimizer_configs.\n"
                "Check your model_config.json structure under training.optimizer_configs"
            )
        
        diffusion_opt_config = self.optimizer_configs['diffusion']
        
        opt_config = diffusion_opt_config['optimizer']
        optimizer = create_optimizer_from_config(opt_config, self.residual_modules.parameters())
        
        print(f"✓ LoRA Optimizer created: {opt_config['type']} with lr={opt_config['config']['lr']}")
        if 'weight_decay' in opt_config['config']:
            print(f"  Weight decay: {opt_config['config']['weight_decay']}")
        if 'betas' in opt_config['config']:
            print(f"  Betas: {opt_config['config']['betas']}")
        
        # Create scheduler if specified in config
        if "scheduler" in diffusion_opt_config:
            sched_config = diffusion_opt_config['scheduler']
            
            # Check if it's CosineAnnealingWithWarmup using SequentialLR
            if sched_config['type'] == 'CosineAnnealingWithWarmup':
                ...
                warmup_scheduler = LinearLR(
                    optimizer,
                    start_factor=1e-10,  # Start near 0 (can't be exactly 0)
                    end_factor=1.0,      # End at base_lr
                    total_iters=warmup_steps
                )
                
                # Phase 2: Cosine annealing from base_lr to eta_min
                ...
                print(f"✓ LoRA Scheduler created: CosineAnnealingWithWarmup (SequentialLR)")
                print(f"  Phase 1 - Linear Warmup: 0 → base_lr over {warmup_steps} steps")
                print(f"  Phase 2 - Cosine Annealing: base_lr → {eta_min} over {cosine_steps} steps")
                print(f"  Total steps: {max_steps}")
                
            else:
                # Use stable-audio-tools function for other schedulers (InverseLR, etc.)
                scheduler = create_scheduler_from_config(sched_config, optimizer)
                print(f"✓ LoRA Scheduler created: {sched_config['type']}")
                if 'warmup' in sched_config['config']:
                    print(f"  Warmup: {sched_config['config']['warmup']}")
            
            # Return in PyTorch Lightning format: ([optimizers], [scheduler_configs])
            scheduler_config = {
                "scheduler": scheduler,
        ...
        # If no scheduler in config, return just optimizer
        print("  No scheduler specified in config")
        return [optimizer]
...
def create_lora_from_config(config, model):
    ...
    # Extract optimizer_configs from training section
    optimizer_configs = None
    if "training" not in config or "optimizer_configs" not in config["training"]:
        raise ValueError(
            "LoRA training requires 'optimizer_configs' in model config.\n"
            "Add 'training.optimizer_configs' section to your model_config_lora.json"
        )

    optimizer_configs = config["training"]["optimizer_configs"]
    print("✓ Loaded optimizer_configs from model config for LoRA training")
