...
def plot_hip_foot_distances(landmark_data, save_path=None):
    ...
    # Calculate Hip-Ankle distances  
    left_hip_ankle_distance = calculate_euclidean_distance(
        landmark_data['LEFT_HIP']['x'], landmark_data['LEFT_HIP']['y'],
        landmark_data['LEFT_ANKLE']['x'], landmark_data['LEFT_ANKLE']['y']
    )

    right_hip_ankle_distance = calculate_euclidean_distance(
        landmark_data['RIGHT_HIP']['x'], landmark_data['RIGHT_HIP']['y'],
        landmark_data['RIGHT_ANKLE']['x'], landmark_data['RIGHT_ANKLE']['y']
    )
    ...
    # Hip-Ankle distances (FIXED LABELS)
    axes[1].plot(landmark_data['LEFT_HIP']['timestamps'], left_hip_ankle_distance, 
                 'b-', label='Left Hip-Ankle Distance', linewidth=2)
    axes[1].plot(landmark_data['RIGHT_HIP']['timestamps'], right_hip_ankle_distance, 
                 'r-', label='Right Hip-Ankle Distance', linewidth=2)
    axes[1].set_title('Hip-Ankle Distance Changes', fontsize=14)
    ...
    return left_hip_heel_distance, right_hip_heel_distance, left_hip_ankle_distance, right_hip_ankle_distance
...
def main():
    ...
    # Set video path
    video_path = "./test_videos/walk5.mp4"
    base_output_dir = "/Users/yejinbang/Documents/GitHub/sfx-project/output/gait_analysis"
    ...
    try:
        ...
        print(f"Total frames: {len(result['landmarks_data'])}")
        ...
        for landmark in landmarks_to_analyze:
            t, x, y, conf = extract_landmark_coordinates(
                result['landmarks_data'], result['timestamps'], landmark
            )
        ...
        # 4. Hip-Foot distance visualization (FIXED VARIABLE NAMES)
        print("\n4. Creating Hip-Foot distance visualization...")
        left_hip_heel, right_hip_heel, left_hip_ankle, right_hip_ankle = plot_hip_foot_distances(
